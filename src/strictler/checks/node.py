...
| `${ref.<id>}` 가 등록소에 실재하는지 | `STR-REG-002` |
...
`(None, [])` 를 준다. 억지로 전개하면 `STR-REF-007`(미해결 참조)이 나면서
...
def findings_of(exc: StrictlerError, *, path: str, node: str = "") -> list[Finding]:
    """`StrictlerError` 를 `Finding` 목록으로 바꾼다.
...
def resolve_script(
    ...
    **`config` 를 안 주면 `${config.X}` 는 "아직 모름"이다** — `(None, [])`.
    ...
    if has_unresolved_config(resolved):
        # Spec 이 채울 값이다. 지금 전개하면 STR-REF-007 이 나면서 정상적인
        # 비교 노드를 등록조차 못 하게 된다.
        return None, []
...
def _resolve_registered(
    ...
    try:
        ...
        return None, [
            rules.finding(
                "STR-REG-002", path=where, node=node.info.name, fields={"id": entry_id}
            )
        ]
    ...
    if not path.is_file():
        return None, [
            rules.finding(
                "STR-REF-001", path=where, node=node.info.name, fields={"script": str(path)}
            )
        ]
    return path, []
