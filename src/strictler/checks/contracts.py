...
| **② 등록소 캐시** | 해시가 그대로인 동안 | **그 파일 바이트만** | `$STRICTLER_HOME/cache/<id>.json` |
...
| 안 하는 것 | 왜 |
|---|---|
| `check_script` 의 `Finding` 들 | `STR-DEP-*` 는 **지금 환경에 그 패키지가 깔려 있나**를 본다. 파일 바이트만으로 정해지지 않는다 |
...
class ScriptCache:
    ...
    def _load(self, path: str, digest: str) -> ScriptContract | None:
        ...
        if not entry_id or registered != digest:
            ...
            # 그 사실은 `STR-REG-001` 이 따로 잡는다 — 여기서는 그냥 다시 뽑는다.
            return None
