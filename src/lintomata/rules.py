...
_TABLE: tuple[Rule, ...] = (
    ...
    _rule(
        ...
        "노드 스크립트는 lintomata 와 **같은 프로세스**에 로드되므로 `import` 가 "
        "lintomata 가 설치된 환경에서 풀립니다. 격리 환경을 만들어 주지 않으니 "
        "그 환경에 함께 설치하세요: {install}\n"
        "⚠ `--with` 는 **선언적**이라 적은 것만 남습니다 — 위 명령은 등록소에 선언된 "
        "것을 전부 포함하고 있으니 **그대로** 쓰세요. 일부만 적으면 나머지가 지워집니다",
    ),
    _rule(
        "LNT-DEP-002",
    ...
    _rule(
        ...
        "`lintomata node test <id>` 로 부르면 **그 id 의 노드가 정본**입니다. "
    ...
    _rule(
        ...
        (_P, _R),
        "참조가 등록소에 없습니다: {id}",
        "참조한 id 가 없습니다 (삭제됐거나 오타). `lintomata list` 로 확인하세요: {id}",
    ),
    _rule(
        "LNT-REG-003",
        "ref-kind-mismatch",
    ...
    _rule(
        ...
        "스크립트의 `from lintomata_lib import <이름>` 은 **능력 선언**("
    ...
    _rule(
        ...
        "노드의 `libraries` 는 스크립트가 `from lintomata_lib import <이름>` 으로 "
    ...
    _rule(
        ...
        "library-import-form",
        (_N,),
        "`lintomata_lib` 를 허용되지 않은 형태로 import 했습니다: {form}",
        "허용되는 형태는 **모듈 최상단의 `from lintomata_lib import <이름>`** "
        "하나뿐입니다. `import lintomata_lib` 나 `from lintomata_lib.<x> import y`, "
