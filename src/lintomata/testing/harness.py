...
| | 확인 | 실패하면 |
|---|---|---|
| 1 | `args` 가 스크립트의 `Args` 선언에 맞는가 | **테스트 정의가 잘못됨** (`LNT-TEST-001`) |
| 2 | `runNode` 가 예외 없이 끝나는가 | 오류 (`LNT-TEST-002`) |
| 3 | `returnResult()` 반환값이 선언된 출력 타입에 맞는가 | 오류 (`LNT-TEST-003`) |

**1번을 따로 세는 이유:** 스크립트가 아니라 **테스트 쪽이 틀린 경우**라 에러 메시지가
달라야 한다. AI 가 fixture 를 잘못 썼는지 스크립트를 잘못 썼는지 구분되어야 자기 수정이 된다.

`expect` 를 준 케이스는 값 대조까지 간다 (`LNT-TEST-004`). 안 주면 타입만 본다 —
...
from lintomata.checks.node import check_node, findings_of, load_node, resolve_script, shape_findings
...
def load_node_test(path: Path, env: Mapping[str, str]) -> tuple[NodeTest | None, list[Finding]]:
    ...
    **JSON 으로 읽히지도 않으면 `LintomataError`** — 위반이 아니라 도구가 못 돈 것이다.
...
def run_node_test(
    ...
    **요청한 것과 다른 노드를 돌리고 `[pass]` 를 냈다.** 통과했다고 보고하는데
    ...
    `node` 필드는 **대조용으로만** 쓰고 어긋나면 `LNT-TEST-008` 이다.
    ...
    **실패는 최대한 모은다.** 한 케이스가 실패해도 나머지 케이스는 전부 돈다.
    ...
    if node_id:
        ...
        # 그 상태에서 `LNT-TEST-008` 을 내면 엉뚱한 곳(테스트의 `node` 필드)을 고치게 된다.
    ...
    # 삭제된 id 는 `resolve_script` 가 `LNT-REG-002` 로 짚는다.
...
def check_requested_node(
    ...
    """테스트의 `node` 가 **요청한 노드**를 가리키는지 대조한다 (`LNT-TEST-008`).
    ...
    **대조뿐이다** — 실행 대상을 고르지 않는다. 그래서 `node` 를 실행 대상 경로로
    ...
    if refs.is_ref(declared):
        ...
        except LintomataError as exc:  # pragma: no cover - 로드 단계가 이미 잡는다
...
def _resolve_node_file(
    ...
    거기서 `LNT-REG-002`(삭제된 id) / `LNT-REG-001`(등록 이후 직접 수정) 이 나온다.

    경로면 경로 규칙으로 푼다. 없으면 `LNT-REF-002` — *"`source` 가 가리키는 노드를
    ...
    if not path.is_file():
        return None, [rules.finding("LNT-REF-002", path=value, fields={"source": str(path)})]
    return path, []
...
def run_case(
    ...
    *,
    ...
    try:
        ...
        return None, [rules.finding("LNT-TEST-002", node=who, fields={"exc": exc.message})]
...
def materialize_args(
    ...
    **`Args` 인스턴스를 만드는 것은 `engine.exec.build_args` 다.** 그건 스크립트가
    ...
    읽기 실패는 **테스트 정의가 잘못된 것**이라 `LintomataError` 로 던진다 —
    부르는 쪽이 `LNT-TEST-001` 로 바꾼다.
...
def check_reckon_contrast(
    ...
    | 대조쌍 자체가 없다 | `LNT-TEST-006` (경고 = `violation`) |
    | 대조쌍은 있는데 판정이 전부 같다 | `LNT-TEST-007` (오류) — **기댓값 하드코딩** |
...
def _fixture_finding(node: str, detail: str) -> Finding:
    """`LNT-TEST-001` — **스크립트가 아니라 테스트 정의가 잘못된 경우.**"""
    return _with_detail(rules.finding("LNT-TEST-001", node=node, fields={}), detail)
