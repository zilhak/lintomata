...
`schema.md` 7절이 근거다. 이름이 `registry` 지만 **등록소(`lintomata.store`)와 무관하다** —
이건 *타입 등록기*다.
...
(합집합이 성립하지 않는 경우 — 같은 필드명의 타입이 갈릴 때 — 는 `LNT-TYPE-006`)
...
class TypeRegistry:
    ...
    def normalize(self) -> None:
        ...
        순환 참조(A 가 B 를, B 가 A 를 필드로)가 있으면 `LNT-TYPE-007`.
        """
    ...
    def _topo_order(self) -> list[TypeKey]:
        """의존하는 것이 먼저 오도록 정렬한다. 순환이면 `LNT-TYPE-007`."""
        order: list[TypeKey] = []
    ...
    def _unknown_type_error(self, t: TypeRef, owner: TypeKey) -> LintomataError:
        """미지 타입 — `LNT-TYPE-003`(unsupported-type).

        `checks/script.py` 가 등록 전에 1차로 잡지만, **2선 방어에도 규칙 id 는 있어야 한다.**
        id 없는 맨 예외로 나가면 리포트에서 무엇이 걸렸는지 기계적으로 알 수 없다.

        ⚠ `LNT-TYPE-003` 은 슬롯 `{type}`·`{file}` 을 갖는다 — 안 채우면 `rules.finding()`
    ...
    def same_definition(self, a: TypeKey, b: TypeKey) -> bool:
        ...
        파이프라인 배선 검사(`LNT-TYPE-004`)가 이걸 쓴다. 이름·origin 이 달라도
    ...
    def _union_fields(self, members: list[TypeKey]) -> dict[str, _ResolvedType]:
        """연결 성분 하나의 필드 합집합. 같은 필드명의 타입이 갈리면 `LNT-TYPE-006`.
...
def _rule_error(
    ...
    *,
    ...
    """규칙 id 가 붙은 `LintomataError` 를 만든다.

    타입 등록기가 내는 것은 전부 **오류**다 — 위반이 아니다. 하지만 규칙 id 없이
    맨 예외로 나가면 리포트에서 무엇이 걸렸는지 기계적으로 알 수 없으므로,
    `Finding` 을 함께 실어 보낸다 (`errors.LintomataError.findings`).

    `message` 를 주면 예외 문구로 그걸 쓴다 — 슬롯 없는 규칙(`LNT-TYPE-003`)은
    ...
    return LintomataError(message or found.message or rule_id, [found])
