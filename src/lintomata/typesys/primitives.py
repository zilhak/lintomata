...
- **`dict` 금지** (`LNT-TYPE-001`) — 허용하면 "복합은 dataclass 강제"가 한 줄로 무의미해진다
- **`None` / `Optional` 금지** (`LNT-TYPE-002`) — "있을 수도 없을 수도"가 부분집합 규칙과
  겹쳐 판정이 흐려진다
- `bytes` 는 스크린샷, `list[T]` 는 버튼 목록 때문에 필요하다

**파이프라인 `config` 선언의 `type` 도 같은 어휘를 쓴다** (`LNT-TYPE-005`) —
타입 어휘를 두 벌 두지 않는다.
"""
...
_FORBIDDEN_RULE: dict[str, str] = {
    "dict": "LNT-TYPE-001",
    "Dict": "LNT-TYPE-001",
    "Optional": "LNT-TYPE-002",
    "None": "LNT-TYPE-002",
    "NoneType": "LNT-TYPE-002",
    "Any": "LNT-TYPE-003",
}
...
def parse_type(expr: str) -> TypeRef:
    ...
    `LNT-TYPE-002` 같은 제대로 된 규칙 대신 도구 오류가 나기 때문이다.
    """
    text = expr.strip()
    if not text:
...
def check_allowed(t: TypeRef, *, known: frozenset[str], path: str, node: str = "") -> list[Finding]:
    ...
    `known` 도 아니면 `LNT-TYPE-003`, `dict` 면 `LNT-TYPE-001`,
    `Optional`/`None` 이면 `LNT-TYPE-002`.

    ★ **세 규칙 전부 자리표시자를 갖는다** — `-001`/`-002` 는 `{file}`, `-003` 은
    `{type}` + `{file}`. 하나라도 빠뜨리면 `rules.finding()` 이 `LintomataError` 를
    ...
    if t.name == _UNION:
        # `str | None` 은 안쪽 `None` 이 LNT-TYPE-002 를 낸다. 그 밖의 합집합은 어휘 밖이다.
        inner: list[Finding] = []
        for arg in t.args:
            inner.extend(check_allowed(arg, known=known, path=path, node=node))
        return inner or [_finding("LNT-TYPE-003", t, path, node)]
    ...
    if base in known and not t.args:
        return []
    return [_finding("LNT-TYPE-003", t, path, node)]
