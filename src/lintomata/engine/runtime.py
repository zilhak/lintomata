...
from lintomata.checks.node import dedupe, findings_of
...
def run_spec(
    ...
    *,
    ...
    """Spec 하나를 실행해 리포트를 만든다. `lintomata check <spec-id>` 의 본체.
    ...
    **한 `plan` 항목이 실패해도 다른 항목은 전부 돈다.**
    ...
    if tool_findings:
        # `tool` 은 Spec 전체에 걸린 선언이다 — 경로가 안 서면 `LNT-TOOL-002` 대조가
...
def _run_compare(
    ...
    **리포트 출력 위치는 Spec `plan` 항목의 `report` 다** — 없으면 `LNT-CMP-001`.
    비교 파이프라인은 실행과 동시에 결과 리포트를 쌓으므로 쌓을 자리가 없으면
    실행할 이유가 없다.

    **`recheck_resolved` 를 여기서도 부른다** (MODULES.md R3-4). `LNT-CMP-002` 와
    target 별 스크립트의 계약·금지 검사는 config 가 풀린 지금이 **유일한 자리**다 —
    비교 파이프라인이야말로 그 재검이 신설된 이유다.
    """
    if not report_target:
        return [rules.finding("LNT-CMP-001", path=path)]
    try:
...
def run_pipeline(
    ...
    **조건 분기는 엔진 문법이 없다** — "앞단 결과가 이러면 아무것도 안 한다"는
    ...
    `tool` 은 Spec 의 `tool` 선언이다 — `LNT-TOOL-001`/`-002` 는 **실행 시점** 규칙이라
    파이프라인만으로는 판정할 수 없다.
...
def _load_nodes(
    ...
    **등록소 무결성은 여기서 본다** — 해시가 등록 당시와 다르면 `LNT-REG-001`.
