...
값의 의미 검증(경로 규칙, 참조 무결성, 타입 계약, 도달 가능성)은 `lintomata.checks.*` 가 한다.
...
`__` 접두는 엔진 예약이라 사용자가 `Args.state` 에 선언할 수 없고(`LNT-STATE-001`),
그래서 **선언 없이 읽어도 `LNT-BAN-004` 가 아니다.**

★ **정본은 여기 하나다.** `engine/state.py`(`ENGINE_FIELDS`) · `refs.py` · `checks/script.py`
셋이 여기서 가져다 쓴다. 복제해 두면 엔진 제공 필드가 늘 때 `LNT-BAN-004` 오탐이 난다.
`model` 은 최하층이라 세 곳 어디서 import 해도 순환이 없다."""

LIBRARY_NAMESPACE = "lintomata_lib"
"""배선된 라이브러리가 들어오는 네임스페이스 (`schema.md` 6.5절).

    from lintomata_lib import buttons          # 스크립트 — **능력 선언**
...
class ToolDecl(BaseModel):
    ...
    실행파일 경로**를 대조하는 정도다. 인자 전체를 검증하려 들면 lintomata 가
    ...
    """스크립트가 호출해도 되는 함수명 목록. 여기 없는 함수를 부르면 `LNT-TOOL-001`."""
...
class ConfigDecl(BaseModel):
    ...
    `bytes` `list[T]`. 타입 어휘를 두 벌 두지 않는다. 허용 집합 밖이면 `LNT-TYPE-005`.
    (`list[T]` 처럼 매개변수를 갖는 표기가 있어 Literal 이 아니라 `str` 이고,
    어휘 검증은 `lintomata.typesys.primitives` 가 한다)"""
...
class States(BaseModel):
    ...
    values: list[str]
    """사용자 정의 상태 이름들. `__` 접두 금지 (`LNT-STATE-001`) — 엔진 제공 필드 전용."""
...
class Transition(BaseModel):
    ...
    **`transitions` 는 시간만 다룬다.** 노드 결과에 따른 분기 문법은 존재하지 않는다
    ...
    to: str
    """이 상태로. `states.values` 에 있어야 한다 (`LNT-STATE-005`)."""
...
class When(BaseModel):
    ...
    state: str
    """스크립트가 `Args.state` 에 선언한 필드 이름 (`LNT-STATE-004`)."""
...
class Pipeline(BaseModel):
    ...
    개수 제한이 없다 — 둘도 되고 열도 된다. 단 2개 미만이면 `LNT-CMP-003`."""
...
class Node(BaseModel):
    ...
    스크립트의 `from lintomata_lib import buttons` 가 *"`buttons` 슬롯이 필요합니다"*
    ...
    ⚠ **슬롯 이름은 스크립트가 정한다.** 여기서 마음대로 지으면 `LNT-LIB-002` 다."""
