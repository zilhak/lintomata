...
| 검증 | 규칙 |
|---|---|
| `source` 가 가리키는 노드가 실재하는지 | `LNT-REF-002` |
| `${ref.<id>}` 접두가 맞는지 / 등록소에 있는지 | `LNT-REG-003` / `LNT-REG-002` |
| `inputs` 가 가리키는 노드 id 가 실재하는지 | `LNT-REF-003` |
| DAG 에 순환이 없는지 / 고립 노드가 없는지 | `LNT-GRAPH-001` / `-002` |
| 등록된 dataclass 집합 정규화 후 **input 정의 == output 정의**인지 | `LNT-TYPE-004` |
| 노드 `states` 매핑이 빠짐없고 대상이 `states.values` 에 실재하는지 | `LNT-STATE-002` / `-003` |
| `when` 이 참조하는 상태 id 가 스크립트의 `Args.state` 에 선언됐는지 | `LNT-STATE-004` |
| `transitions.after` 가 실재 노드 id 인지, `to` 가 실재 상태인지 | `LNT-REF-004` / `LNT-STATE-005` |
| 사용자 상태 이름에 `__` 접두가 없는지 | `LNT-STATE-001` |
| `config` 선언의 `type` 이 primitive 집합에 속하는지 | `LNT-TYPE-005` |
| `kind: compare` 일 때 `compare` 가 가리키는 노드가 실재하는지 | `LNT-REF-005` |
| `targets` 가 2개 이상인지 | `LNT-CMP-003` |

**target 별 스크립트 대조(`LNT-CMP-002`)는 등록 시점에 못 한다** — 스크립트를 가르는
`${config.X}` 를 채우는 것이 Spec 이기 때문이다. 그 검사와, 그때야 정해지는 스크립트의
계약·금지 검사는 **Spec 실행 시점의 `recheck_resolved`** 가 한다 (MODULES.md R3-4).

실행 시점의 config 채움(`LNT-CONFIG-001`~`003`)과 `path: true` 검증(`LNT-PATH-004`)도
여기 산다 — **`config` 를 선언한 것이 파이프라인이므로 그 값을 판정하는 것도 여기다**
(MODULES.md R1-6). `refs.expand_config` 는 **default 가 이미 채워진** config 를 받는다.

도달 가능성(`LNT-STATE-006` / `-007`)은 `checks.reachability` 가 맡는다.
...
from lintomata.checks import reachability, script as script_checks
...
def check_cycle(
    ...
    """순환 판정 (`LNT-GRAPH-001`). 순환 경로를 메시지에 넣는다.

    고립 노드(`LNT-GRAPH-002`)도 여기서 함께 본다.
    ...
    def visit(nid: str) -> None:
        ...
        for dep in dag.get(nid, ()):
            if dep not in dag:
                continue  # 없는 노드 참조는 LNT-REF-003 이 따로 잡는다
    ...
    if len(dag) > 1:
        ...
        for nid, deps in dag.items():
            if not deps and nid not in consumed and nid not in exempt:
                findings.append(
                    rules.finding(
                        "LNT-GRAPH-002", path=source_path, node=nid, fields={"name": nid}
                    )
                )
    return findings
...
def check_ambiguous_input(pipeline: Pipeline, source_path: str) -> list[Finding]:
    """한 노드의 `inputs` 가 **서로 다른 앞단**을 둘 이상 가리키는지 (`LNT-GRAPH-003`).
...
def check_wiring_types(
    ...
    *,
    ...
    """배선된 두 노드의 **선언된 정의가 동일한지** (`LNT-TYPE-004`).
    ...
    **매핑 누락·존재하지 않는 상태 참조를 로드 시점에 잡는다** (자기 검증적 스키마).
    ...
    for name in pipeline.states.values:
        if name.startswith("__"):
            findings.append(
                rules.finding("LNT-STATE-001", path=source_path, fields={"name": name})
            )
...
def check_transitions(pipeline: Pipeline, source_path: str) -> list[Finding]:
    ...
    for transition in pipeline.transitions:
        if transition.after not in ids:
            findings.append(
                rules.finding(
                    "LNT-REF-004", path=source_path, fields={"name": transition.after}
                )
            )
        if transition.to not in values:
            findings.append(
                rules.finding(
                    "LNT-STATE-005", path=source_path, fields={"name": transition.to}
                )
            )
    return findings
...
def check_config_decls(pipeline: Pipeline, source_path: str) -> list[Finding]:
    """`config` 선언의 `type` 이 primitive 어휘에 속하는지 (`LNT-TYPE-005`).
...
def check_config_values(
    ...
    *,
    ...
    `LNT-CONFIG-001`(required 누락) / `-002`(타입 불일치) / `-003`(선언 없는 키) /
    `LNT-PATH-004`(`path: true` 인데 경로 규칙 위반).

    **`default` 주입이 여기 책임이다** (MODULES.md R1-6). `refs.expand_config` 가
    받는 config 는 이미 default 가 채워진 것이어야 한다 — 그래야 거기서 못 찾은
    `${config.X}` 가 **진짜 required 누락**이 되고 `LNT-CONFIG-001` 재사용이 정당해진다.
    ...
    **주입한 `default` 도 같은 대조를 받는다** (`node="default"` 로 표시).
    ...
    통과한다. `LNT-TYPE-005` 는 `type` **표기**가 어휘 밖일 때의 자리라
    (guide 가 어휘를 고치라고 말한다) 값 불일치에는 `LNT-CONFIG-002` 가 맞다.
    """
    findings: list[Finding] = []
    ...
    for key in values:
        ...
        if key not in pipeline.config:
            findings.append(
                rules.finding("LNT-CONFIG-003", path=source_path, fields={"name": key})
            )
    ...
    for name, decl in pipeline.config.items():
        ...
        if name in values:
            ...
            findings.append(
                rules.finding("LNT-CONFIG-001", path=source_path, fields={"names": name})
            )
...
def _matches(t: TypeRef, value: Any) -> bool:
    ...
    if t.name not in PRIMITIVES:
        return True  # 어휘 밖 — LNT-TYPE-005 가 따로 잡는다
    if t.name == "bool":
...
def check_compare(
    ...
    """비교 파이프라인 전용 검사 (`LNT-REF-005` / `LNT-CMP-002` / `-003`).
    ...
    **대상 개수에 제한이 없다.** 짝지어 비교하는 것이 아니라 **목록 전부가 같은지**를
    ...
    if len(pipeline.targets) < 2:
        findings.append(
            rules.finding(
                "LNT-CMP-003", path=source_path, fields={"count": len(pipeline.targets)}
            )
        )
    ...
    for name in pipeline.compare:
        if name not in ids:
            findings.append(
                rules.finding("LNT-REF-005", path=source_path, fields={"name": name})
            )
...
def _check_target_types(
    ...
    """target 별 스크립트의 input/output/state 타입이 **전부** 같은지 (`LNT-CMP-002`)."""
...
) -> tuple[bool, LintomataError | None]:
...
def build_registry(
    ...
    **키가 `(origin, name)`** 이므로 노드마다 있는 `Args` 가 충돌하지 않는다.
    정규화 중 순환 참조(`LNT-TYPE-007`)·미지 타입(`LNT-TYPE-003`)이 나오면 그때
...
def check_pipeline(
    ...
    **실패를 최대한 모은다.** 한 노드가 깨져도 나머지는 전부 검사한다 —
    ...
    # 채우는 것이 Spec 이기 때문이다. `LNT-CMP-002` 는 `recheck_resolved` 가 낸다
    # (MODULES.md R3-4). 여기서 보는 것은 config 와 무관한 `LNT-REF-005`/`-CMP-003` 다.
...
def _load_referenced_node(
    ...
    *,
    ...
    if not path.is_file():
        return None, [
            rules.finding(
                "LNT-REF-002", path=source_path, node=node_id, fields={"source": value}
            )
        ]
...
def recheck_resolved(
    ...
    *,
    ...
    - target 별 스크립트가 같은 input/output/state 를 선언했는지(`LNT-CMP-002`)를
      **판정할 수 없고**,
...
def _representative(
    ...
    `LNT-CMP-002` 가 이미 냈다. 여기서 target 마다 다시 대조하면 같은 배선 결함이
