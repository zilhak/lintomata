...
| 다른 라이브러리 `import` | (배선된 것만) | ❌ **금지 — 한 층만** (`LNT-LIB-003`) |
| `dataclass` 선언 | ✅ | ❌ **v1 금지** (`LNT-LIB-004`) |
...
⚠ **`lintomata library test` 는 없다.** 라이브러리엔 `runNode` 계약이 없어 지금
...
from lintomata import deps, refs, rules
...
라이브러리 **안에서** 이 이름이 보이면 그건 라이브러리 중첩이다 (`LNT-LIB-003`).
스크립트 안에서 보이는 것은 정상이고, 형태만 `LNT-LIB-005` 가 본다."""
...
def check_library(
    ...
    **실패를 최대한 수집한다** — 하나 걸렸다고 나머지를 멈추지 않는다.
    ...
    파싱이 안 되는 것은 위반이 아니라 검사기가 못 돈 것이므로 `LintomataError` 다
...
def check_no_nesting(tree: ast.Module, path: str) -> list[Finding]:
    """라이브러리가 다른 라이브러리를 쓰지 않는지 (`LNT-LIB-003`).

    **형태를 가리지 않는다** — `import lintomata_lib` 든
    `from lintomata_lib import x` 든 라이브러리 안에서는 전부 중첩이고, 고치는 법도
    하나다: 그 import 를 없앤다. (스크립트 쪽의 *형태* 문제는 `LNT-LIB-005` 다 —
    ...
    for name in _namespace_uses(tree):
        if name in seen:
            continue
        seen.add(name)
        findings.append(rules.finding("LNT-LIB-003", path=path, fields={"name": name}))
    return findings


def check_no_dataclass(tree: ast.Module, path: str) -> list[Finding]:
    """라이브러리가 `dataclass` 를 선언하지 않는지 — **v1 제한** (`LNT-LIB-004`).
    ...
    """
    return [
        rules.finding("LNT-LIB-004", path=path, fields={"name": stmt.name})
...
def check_wiring(
    ...
    """스크립트가 요구한 슬롯과 노드가 배선한 슬롯이 맞는지 (`LNT-LIB-001` / `-002`).
...
def resolve_libraries(
    ...
    **2절의 "ref 는 로컬 최적화, 경로는 이식 가능" 이 그대로 적용된다** — 커밋할
    ...
    | 잘못된 것 | 규칙 |
    |---|---|
    | `${ref.sc_...}` 처럼 **라이브러리가 아닌 것**을 배선했다 | `LNT-REG-003` |
    | 그 id 가 등록소에 없다 | `LNT-REG-002` |
    | 가리키는 파일이 없다 | `LNT-REF-001` |
    | 경로 규칙 위반 | `LNT-PATH-001`~`003` |

    **`LNT-REG-003` 을 새로 파지 않았다** — *"이 자리에는 X 가 와야 하는데 Y 를 줬다"*
...
def _resolve_one(
    ...
    if refs.is_ref(value):
        ...
        try:
            ...
            return None, [rules.finding("LNT-REG-002", fields={"id": entry_id})]
    ...
    if not path.is_file():
        return None, [rules.finding("LNT-REF-001", fields={"script": str(path)})]
    return path, []


def _findings_of(exc: LintomataError) -> list[Finding]:
...
def _namespace_uses(tree: ast.Module) -> list[str]:
    """`lintomata_lib` 를 건드리는 import 를 등장 순서대로 **원문 그대로** 모은다."""
    found: list[str] = []
