...
> 라이브러리 검사 결과가 노드의 `info.name` 을 달고 나가 **같은 노드가 리포트에 두
> 이름으로 찍혔다** (`detect-buttons` / `detectButtons`). 이름이 갈리면 같은 것인지
> 알 수 없을 뿐 아니라, `not run` 전파가 **노드 id 문자열로 대조**하므로 여파가 통째로
> 어긋난다.
> - **발신처(`checks/library.py`·`checks/node.check_libraries`)는 `node` 를 안 채운다.**
> - **부르는 쪽이 노드 id 로 덮어쓴다 — `or` 가 아니다.** 자리는 둘:
>   `checks/pipeline._resolved_contract`(정적 재검) 과 `engine/drive.resolve_libraries`(실행).
>   한쪽만 고치면 다시 갈린다.
> - 예외는 `testing/harness.py` 다 — 거기엔 파이프라인 노드 id 가 없으므로 `info.name` 이 맞다.
>
> **그리고 못 푼 노드는 돌리지 않는다.** `runtime._load_nodes` 와 `compare._prepare` 가
> **같은 처리**를 한다(둘 다 `continue`). 억지로 로드하면 스크립트가 `ImportError` 로 죽으면서
> *"배선이 없습니다"* 라는 **거짓 안내**가 진짜 원인(파일이 없다) 위에 덮인다. 여파는 `not_run` 이다.
