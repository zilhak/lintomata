...
- **인용한 출력은 `--lang ko` 기준이다.** 기본 출력 언어는 **영어**다(`schema.md` 2절) —
  그냥 돌리면 같은 자리에 영어 문장이 나온다. 문서와 글자까지 맞춰 보려면 `--lang ko` 를 붙이거나
  `$LINTOMATA_HOME/config.json` 에 `{"locale": "ko"}` 를 둔다.
  **언어가 바꾸는 것은 사람이 읽는 문장뿐이다** — 규칙 id·개수 줄·종료 코드는 언제나 같다.
  그러니 AI 가 자기 수정의 근거로 삼을 것은 **규칙 id** 이지 문장이 아니다.
...
여기까지가 기획이고, 나머지는 이 문장을 네 수준으로 쪼개는 기계적인 작업이다.

| 묻는 것 | 답이 곧 | 예제 |
|---|---|---|
| **어떤 환경에서 뽑을 것인가?** | **Prepare** | `targets/home.html` 파일 하나를 관측 환경으로 잡는다 |
...
- **Act** — 추출도 판정도 아닌 **행위**다. 클릭·입력·기록. `input == output` 이라
